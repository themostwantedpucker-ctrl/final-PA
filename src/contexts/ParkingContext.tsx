...
export const ParkingProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Passive device polling: periodically refresh from server to mirror admin
  useEffect(() => {
    const interval = setInterval(async () => {
      if (document.visibilityState !== 'visible') return;
      // Do not poll on the active operator device
      const isActiveOperator = localStorage.getItem('activeOperator') === 'true';
      if (isActiveOperator) return;
      try {
        // Vehicles
        const freshVehiclesRaw = await loadVehicles();
        const freshVehicles = freshVehiclesRaw.map(v => ({
          ...v,
        ...
        const vehiclesKey = (arr: typeof vehicles) => JSON.stringify(
          [...arr].map(v => ({ id: v.id, exit: !!v.exitTime })).sort((a, b) => a.id.localeCompare(b.id))
        );
        if (vehiclesKey(freshVehicles) !== vehiclesKey(vehicles)) {
          setVehicles(freshVehicles);
        }

        // Permanent clients
        const freshClientsRaw = await loadPermanentClients();
        const freshClients = freshClientsRaw.map(c => ({
          ...c,
          entryTime: new Date(c.entryTime),
          exitTime: c.exitTime ? new Date(c.exitTime) : undefined,
          paymentDate: c.paymentDate ? new Date(c.paymentDate) : undefined,
        }));
        const clientsKey = (arr: typeof permanentClients) => JSON.stringify(
          [...arr].map(c => c.id).sort()
        );
        if (clientsKey(freshClients) !== clientsKey(permanentClients)) {
          setPermanentClients(freshClients);
        }

        // Settings
        const freshSettings = await loadSettings();
        if (JSON.stringify(freshSettings) !== JSON.stringify(settings)) {
          setSettings(freshSettings);
        }

        // Daily stats
        const freshStats = await loadDailyStats();
        if (JSON.stringify(freshStats) !== JSON.stringify(dailyStats)) {
  ...
  }, [vehicles, permanentClients, settings, dailyStats]);
  ...
  const exitVehicle = async (vehicleId: string) => {
    ...
    if (vehicle) {
      ...
      try {
        ...
        console.error('Failed to exit vehicle via API:', error);
      }
    }
